...
rx_fifo u_rx_fifo (
	...
	.wr_clk      (clk156), //: IN STD_LOGIC;
	.rd_clk      (clk), //: IN STD_LOGIC;
	.din         ({tkeep_rx_fifo, tdata_rx_fifo}), //: IN STD_LOGIC_VECTOR(519 DOWNTO 0);
	.wr_en       (fifo_wr_en), //: IN STD_LOGIC;
	.rd_en       (rx_fifo_rd_en), //: IN STD_LOGIC;
	.dout        ({tkeep_delay, tdata_delay}), //: OUT STD_LOGIC_VECTOR(519 DOWNTO 0);
	.full        (), //: OUT STD_LOGIC;
    ...
    always @ (*) begin
             ...
             case(state)
                 IDLE: begin
                     ...
                     end else if(i_tvalid && (i_tkeep == 64'hFFFFFFFF_FFFFFFFF)) begin
                         //info_fifo_wr_en = 1'b1;
                         if(~fifo_almost_full & ~info_fifo_full) begin
                             info_fifo_wr_en = 1'b1;
                             fifo_wr_en = 1'b1;
         ...
         case(err_state)
             ERR_IDLE: begin
               err_state_next = ERR_IDLE;
                 ...
                 if (tkeep_delay == 64'h0 & ~fifo_empty) begin // end of the packet 
                     rx_fifo_rd_en = 0;
                     o_tvalid = 0;
...
//ila_5 u_rx_queue_ila (
//	.clk     (clk156),
//	.probe0  (i_tdata),
//	.probe1  (state),
//	.probe2  (i_tvalid),
//	.probe3  (i_tlast),
//	.probe4  (areset_clk156),
//	.probe5  (32'd0),
//	.probe6  (4'd0),
//	.probe7  (1'b0),
//	.probe8  (1'b0),
