...
*/
    ...
    localparam WAIT_BRAM_RDINC = 2;
    localparam WAIT_BRAM_RDZR = 3;
    localparam WAIT_BRAM_ADD = 4;
    localparam WRITE_RESULT = 5;
    localparam WRITE_RESULT_RDTS = 6;
    ...
    // CUSTOM REGISTERS
    reg  [INDEX_WIDTH-1:0]   temp_addr;
    reg  [REG_WIDTH-1:0]     temp_val;
    reg  [REG_WIDTH-1:0]     timestamp;
    reg                      tsflag;
   ...
   always @(*) begin
      ...
      case(d_state)
          START_REQ: begin
              if (~empty_fifo) begin
                  ...
                  if (statefulValid_fifo && index_fifo < REG_DEPTH) begin
                      ...
                      // READ_INCREMENT
                      else if (opCode_fifo == `RDINC_OP) begin
                          d_addr_in_bram = index_fifo;
                          d_addr_in_bram_r_next = index_fifo;
                          temp_addr = index_fifo;
                          tsflag = 0;
                          d_state_next = WAIT_BRAM_RDINC;
                      end
                      // READ_ZERO
                      else if (opCode_fifo == `RDZR_OP) begin
                          d_addr_in_bram = index_fifo;
                          d_addr_in_bram_r_next = index_fifo;
                          temp_addr = index_fifo;
                          tsflag = 0;
                          d_state_next = WAIT_BRAM_RDZR;
                      end
                      // READ_TIMESTAMP
                      else if (opCode_fifo == `RDTS_OP) begin
                          valid_out = 1;
                          result_out = timestamp;
                          tsflag = 1;
                          d_state_next = WRITE_RESULT_RDTS;
                      end
                      // ADD
                          ...
                          d_addr_in_bram_r_next = index_fifo;
                          temp_addr = index_fifo;
                          temp_val = newVal_fifo;
                          tsflag = 0;
                          d_state_next = WAIT_BRAM_ADD;
                      end
                      // READ, IF GT, ZERO
                      else if ((opCode_fifo == `RGTZ_OP) && (timestamp >= newVal_fifo)) begin
                          valid_out = 1;
                          result_out = timestamp;
                          tsflag = 1;
                          d_state_next = WRITE_RESULT_RDTS;
          ...
          WAIT_BRAM_RDINC: begin
              if (cycle_cnt == 1'b1) begin // 2 cycle BRAM read latency
                  cycle_cnt_next = 0;
                  d_we_bram = 1;
                  d_addr_in_bram = temp_addr;
                  d_addr_in_bram_r_next = temp_addr;
                  d_data_in_bram = d_data_out_bram + 1;
                  result_r_next = d_data_out_bram + 1;
                  d_state_next = WRITE_RESULT;
              end
              else begin
                  cycle_cnt_next = cycle_cnt + 1;
          ...
          WAIT_BRAM_ADD: begin
              if (cycle_cnt == 1'b1) begin // 2 cycle BRAM read latency
                  ...
                  d_addr_in_bram = temp_addr;
                  d_addr_in_bram_r_next = temp_addr;
                  d_data_in_bram = d_data_out_bram + temp_val;
                  result_r_next = d_data_out_bram + temp_val;
                  d_state_next = WRITE_RESULT;
              end
              else begin
                  cycle_cnt_next = cycle_cnt + 1;
              ...
              if (cpu2ip_valid_r1 && cpu2ip_index_r1 < REG_DEPTH) begin
                  ...
                  if (ipReadReq_index_r1 < REG_DEPTH) begin
                      if(ipReadReq_index_r1 != 0) begin
                        ...
                        ip2cpu_@PREFIX_NAME@_reg_data_adj = timestamp;
                        ip2cpu_@PREFIX_NAME@_reg_index = 0;
